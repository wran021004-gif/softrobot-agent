...
from schemas import platform, platform_math, platform_learning, platform_diagnostics
...
def export():
    ...
    contracts = {name: cls.model_json_schema() for module in (platform, platform_math, platform_learning, platform_diagnostics) for name, cls in vars(module).items()
    ...
    lines = ['# Public capability catalog (generated)', '', 'Generated by `python examples/export_platform_contracts.py`; not an authorization allowlist.', '',
    ...
    lines += [f"| {r['extension_id']} | {r['capabilities'].get('category', 'platform_services')} | {r['capabilities'].get('role', 'adapter')} | {r['kind']} | {r['version']} | {r['implementation_exists']} | {r['binding'] or 'Not implemented'} | {r['description']} |" for r in reg.catalog()]
