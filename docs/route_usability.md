...
| optimize | Optional build/run/optimize source; otherwise original baseline. A matching evaluated source supplies the first proposal's saved evaluation. `variables[path]=[lower,upper]` supplies numerical bounds, not samples. An explicit combination with a source must match its bindings. | Up to the bounded proposal count; matching evaluated starts and duplicate candidates reuse evidence |
...
For an evaluated optimization source, the first proposal reuses its valid evaluation only when the effective configuration, task instance and simulation/evaluation dependencies match. Session IDs, search settings and budget are excluded from the configuration comparison; design, control, task, model, backend and numerical settings remain included. The first proposal still consumes one proposal slot and provides the same coordinate-search feedback. No simulation or evaluation is repeated for that start. `reused_evaluations` and `new_evaluations` are reported separately from ledger-based `actual_solves`. Reused trials retain the original candidate label, `owner_run_id`, configuration, simulation execution and evaluation references; delivery resolves the owner separately from `search_run_id`. New search candidates receive a distinct label if the reused source already owns their default label. This is explicit source reuse, not a global cache.

Finish summaries derive `simulated` and `evaluated` from the saved candidate evidence, while finish reports zero additional solves/evaluations. A build remains unevaluated. Route-internal child calls use the existing Host actor `route-executor`, with existing parent-event/session links retaining the originating route request. Manual entry points retain `local-human`; sealed requests retain their original identity and attribution. Historical terminal records are not rewritten.

The cleanup reviewed at `09fba6c` passed three focused checks in `tests.test_route_usability`: evaluated-start reuse (including a chained optimization, saved-state recovery, preserved coordinate order and changed-design non-reuse), finish status with zero additional work, and internal/manual caller attribution with originating request links. All backend outputs in these checks were synthetic: zero live model requests and zero dynamics integrations. No task tolerance, controller gains, budgets or protocol retry settings changed.
