# Unified tool directory (generated)
...
| --- | --- | --- | --- | --- |
| workbench.inspect_task | 1.1.0 | read_inputs | 解析冻结合同、设计语法及工具适用性 | tools.workbench_actions:execute |
| workbench.analyze_design | 1.1.0 | analysis | 现有 PCC 直态局部雅可比；不判断任务成绩 | tools.workbench_actions:execute |
| workbench.evaluate_design | 1.1.0 | simulate | 原 Harness 完整评价；或请求创建时登记的封存历史运行回放（零后端，单独标明历史成绩）；整次候选为恢复边界 | tools.workbench_actions:execute |
| workbench.diagnose | 1.1.0 | read_evidence | 读取原评价和诊断证据；因果归因保持 UNKNOWN | tools.workbench_actions:execute |
| workbench.observe | 1.1.0 | derived_artifacts | 复用 ObservationViewer；保存数据回放，零动力学调用 | tools.workbench_actions:execute |
| workbench.read_evidence | 1.1.0 | read_evidence | 按 evidence_id 和 JSON Pointer 读取字段或有限片段；catalog:cNNN 的 /entries 为候选文件目录；返回 cite_as 可直接引用 | tools.workbench_actions:execute |
| workbench.create_candidate | 1.1.0 | candidate_design | 只修改权威 envelope 允许的参数；必须引用 parent_id 的本轮评价证据，不能提前生成序列 | tools.workbench_actions:execute |
| workbench.check_candidate | 1.1.0 | analysis | 检查指定候选的语法、范围与关系约束，生成独立 IR 和 PCC 局部分析 | tools.workbench_actions:execute |
| workbench.evaluate_candidate | 1.1.0 | simulate | 指定候选经原 Harness 的 MATLAB M0/M1/形状、C1、编译、MuJoCo 和冻结评价；成功与失败均返回真实数据 | tools.workbench_actions:execute |
| workbench.compare_candidates | 1.1.0 | read_evidence | 仅比较指定候选已有的本轮评价；任务状态与误差来自既有评价程序 | tools.workbench_actions:execute |
| workbench.observe_candidate | 1.1.0 | derived_artifacts | 用原观察器从指定候选保存数据生成动画和曲线，不重新仿真 | tools.workbench_actions:execute |
| dynamics.analyze_pcc | 1.1.0 | analysis | Task-independent analytic PCC tip/Jacobian for explicit length and bend; local geometry only, zero backend solves, no task score. | tools.dynamic_actions:analyze_pcc |
| dynamics.create_candidate | 1.1.0 | candidate_design | Branch any registered candidate, changing design, equivalent physics or control; cite recorded evidence. | tools.dynamic_actions:create_candidate |
| dynamics.simulate_candidate | 1.1.0 | simulate | Run only the selected backend. MATLAB screening does not set canonical reach success. One rollout charged even on failure. | tools.dynamic_actions:simulate_candidate |
| dynamics.evaluate_candidate | 1.1.0 | simulate | Compatibility: evaluate candidate in MuJoCo with its own controller, no bundled MATLAB calls. | tools.dynamic_actions:evaluate_candidate |
| dynamics.optimize_matlab | 1.1.0 | analysis | MATLAB bounded local coordinate search, checkpoint every trial, one dynamic budget unit per actual rollout. Resume identical search_id. Choose a small evidence-based variable subset and a bounded batch. | tools.dynamic_actions:optimize_matlab |
| dynamics.diagnose_trajectory | 1.1.0 | read_evidence | Query exact entity and time from saved trajectory. Force and state have distinct time phases. Returns sampled facts, events and evidence refs. | tools.dynamic_actions:diagnose_trajectory |
| dynamics.compare_candidates | 1.1.0 | read_evidence | Paged design/control/physics/backend comparison. Unevaluated backends remain NOT_RUN. | tools.dynamic_actions:compare_candidates |
| dynamics.observe_candidate | 1.1.0 | derived_artifacts | Render saved MATLAB or MuJoCo coordinates, curves and event list; zero backend solves. | tools.dynamic_actions:observe_candidate |
| dynamics.render_simulation_video | 1.1.0 | derived_artifacts | On demand: render a registered saved result with its native backend to an MP4 file reference, without solving or scoring. Use reason to state a specific visual inspection question. Optional time interval defaults to the saved trajectory. A text model receiving the video reference has NOT seen or visually understood the frames. | tools.dynamic_actions:render_simulation_video |
| dynamics.read_evidence | 1.1.0 | read_evidence | Read registered JSON with pointer, offset, limit and max_bytes. Follow next_read; oversized items return child pointers and resume_container. Array offsets are original source indices. | tools.dynamic_actions:read_evidence |
| dynamics.record_verified_diagnosis | 1.1.0 | read_evidence | Record your concrete diagnostic statement with exact machine-checkable entity, time interval, numeric field and value from a diagnosis record. | tools.dynamic_actions:record_verified_diagnosis |
| dynamics.stop_design | 1.1.0 | read_evidence | Stop with cited results and limitations; distinguish workflow, numerical completion, MATLAB prediction and canonical MuJoCo success. | tools.dynamic_actions:stop_design |
| workbench.stop_design | 1.1.0 | session_control | Record a cited terminal decision with reason; no numerical execution. | tools.workbench:Workbench.submit |
| workbench.capability_missing | 1.1.0 | session_control | Record a cited terminal decision with reason; no numerical execution. | tools.workbench:Workbench.submit |
| analysis.pcc_jacobian | 1.1.0 | analysis | Single section inextensible PCC tip/Jacobian, m and m/rad, base +x, yz bending; norm(bend)<=pi. Geometry only. | tools.public_services:pcc_jacobian |
| evidence.read_json | 1.1.0 | read_evidence | Read hash-verified saved JSON using bounded JSON Pointer pages. | tools.public_services:read_json |
| diagnostics.saved_trajectory | 1.1.0 | read_evidence | Legacy reach signal rules; saved physical time, no simulation or scoring. | tools.public_services:saved_diagnosis |
| visualization.render_simulation_video | 1.2.0 | derived_artifacts | Render saved trajectory; native cache and bounded encoder, zero solves. | tools.simulation_video:_render_simulation_video |
| analysis.pcc_condition | 1.1.0 | analysis | Singular values/rank of the local PCC tip Jacobian; geometric conditioning, no controllability or task claim. | tools.math_extensions:pcc_condition |
| diagnostics.signal_rule | 1.1.0 | read_evidence | Run a versioned saved-signal rule; distinguish missing, no event, inapplicable and failure. | tools.diagnostic_rules:run_saved_rule |
| analysis.pcc_tolerance | 1.0.0 | analysis | Propagate independent length/bend standard deviations to first-order tip covariance, principal error directions and ranked tolerance contributions. Geometric approximation, no task assessment. | tools.pcc_tolerance:analyze |
...
Bundle aliases: `{'matlab_analysis': 'model'}`.
