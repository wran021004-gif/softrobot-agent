...
def _segments(physics, resolved):
    ...
    for local in resolved.segments:
        ...
        for part in parts:
            ...
            center += ds / 2
        matrix = np.vstack([cell_average_basis(resolved, local, *sample['normalized_interval'])
...
def description(physics, design, basis=None):
    ...
    return dict(id=PROJECTOR_ID, version='2.0.0',
                coordinate_order=resolved.coordinate_order,
                resolved_basis=resolved.model_dump(mode='json'), segments=segments,
                reconstruction='segment mean curvature from principal joint angle / actual cell length; least squares on exactly integrated basis split at knots')
...
def discretization_jacobian(physics, design, basis=None):
    ...
    for local, samples, _ in _segments(physics, resolved):
        for sample in samples:
            matrix = cell_average_basis(resolved, local, *sample['normalized_interval'])
