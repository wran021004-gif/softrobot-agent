...
def declarations(tension_reference=False, execution_mode='actuator_realistic'):
    values=[
    ...
    if execution_mode=='actuator_realistic':
        ...
        values.extend([
            ('tendon_length_change','tendon',1,'m','path','post_step','tendon_length_change_m'),
            ('tendon_length_rate','tendon',1,'m/s','path','post_step','tendon_length_rate_m_s')])
...
def expanded(p,tension_reference=False,execution_mode='actuator_realistic'):
    groups=dict(joint=p['dofs'],tendon=[t['entity'] for t in p['tendons']],actuator=[a['id'] for a in p['actuators']],tip=['tip'])
    for item in declarations(tension_reference,execution_mode):
...
def export(rows,p,tension_reference=False,execution_mode='actuator_realistic'):
    return [Signal(spec=s,times_s=[r['time_s' if s.phase=='post_step' else 'solver_time_s'] for r in rows],
        ...
        for item,i,s in expanded(p,tension_reference,execution_mode)]
