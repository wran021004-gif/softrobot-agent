...
def declarations():
    return [dict(name=n,entity_group=g,dimension=d,units=u,frame=f,phase=phase,field=field) for n,g,d,u,f,phase,field in [
...
def observation_specs(inp,reg):
    ...
    return [s for _,_,s in expanded(physics_for(inp))]


def export(rows,p):
    return [Signal(spec=s,times_s=[r['time_s' if s.phase=='post_step' else 'solver_time_s'] for r in rows],
        values=[r[item['field']] if s.dimension==3 else [r[item['field']][i]] for r in rows]) for item,i,s in expanded(p)]
