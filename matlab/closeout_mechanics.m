function output = closeout_mechanics(input_json)
...
try
    ...
    elseif strcmp(p.operation,'dynamic')
        ...
        assert(all(isfinite(x(:))) && max(abs(x(:,1)))<=2.8,'State outside reduced-model domain');
    ...
    r.elapsed_s=toc(timer); r.model='one_mode_discrete_rod_v1';
...
output=jsonencode(r);
    ...
    function dx=rhs(t,x)
        assert(toc(timer)<=30,'Solver timeout');
        assert(abs(x(1))<=3.2,'Reduced-model domain exceeded');
