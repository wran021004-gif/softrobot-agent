...
MATLAB_ADAPTERS = {'matlab_tdcr_planar_dynamic_v1': 'tdcr_planar_saved_v1',
                   'matlab_serial_bending_v1':'family_matlab_saved_v1'}
...
def _source(root, registry, ref, backend):
    ...
    result = read(path)
    family=result.get('data',{}).get('contract')=='family.backend_data'
    actual={'backend.matlab_spatial':'matlab','backend.family_mujoco':'mujoco'}.get(result.get('backend_id')) if family else result.get('backend')
    if actual != backend:
    ...
    required = [path, trajectory]
    if backend=='mujoco' or not family: required.append(folder/'robot.xml')
    if family:
        required += [folder/'resolved_physics.json',folder/'experiment_scene.json']
        if backend=='mujoco': required.append(folder/'compiled_physics.json')
    elif backend == 'matlab':
    ...
    pending = [folder/'robot.xml'] if (folder/'robot.xml').exists() else []; visited = set()
    while pending:
        xml = pending.pop()
...
def _mujoco_frames(folder, rows, indices, frames, azimuth_deg=135., elevation_deg=-20.):
    import mujoco
    from PIL import Image
    from tools.native_replay import NativeReplay
    if (folder/'compiled_physics.json').exists() and (folder/'resolved_physics.json').exists():
        import numpy as np
        model=mujoco.MjModel.from_xml_path(str(folder/'robot.xml')); data=mujoco.MjData(model)
        mapping=read(folder/'compiled_physics.json'); physics=read(folder/'resolved_physics.json')
        qi=[int(model.jnt_qposadr[model.joint(name).id]) for name in physics['dofs']]
        vi=[int(model.jnt_dofadr[model.joint(name).id]) for name in physics['dofs']]
        if qi!=mapping['qpos_indices'] or vi!=mapping['qvel_indices']: raise ValueError('SAVED_JOINT_MAPPING_MISMATCH')
        camera=mujoco.MjvCamera(); mujoco.mjv_defaultCamera(camera)
        camera.lookat[:]=np.asarray(read(folder/'experiment_scene.json')['mount_position'])+[.15,0,0]
        camera.distance=.7; camera.azimuth=azimuth_deg; camera.elevation=elevation_deg
        model.vis.global_.offwidth=max(800,model.vis.global_.offwidth); model.vis.global_.offheight=max(600,model.vis.global_.offheight)
        with mujoco.Renderer(model,height=600,width=800) as renderer:
            for k,index in enumerate(indices):
                data.qpos[qi]=rows[index]['qpos_rad']; data.qvel[vi]=rows[index]['qvel_rad_s']
                mujoco.mj_forward(model,data)  # state/geometry reconstruction, never mj_step
    ...
    player.camera.azimuth=azimuth_deg; player.camera.elevation=elevation_deg
...
def render_simulation_video(root, evidence_registry, result_ref, backend,
                            t_start_s=None, t_end_s=None, fps=25, azimuth_deg=135., elevation_deg=-20.):
    ...
    args=RenderVideo(result_ref=result_ref,backend=backend,t_start_s=t_start_s,t_end_s=t_end_s,fps=fps,
        azimuth_deg=azimuth_deg,elevation_deg=elevation_deg).model_dump()
...
def _render_simulation_video(root, evidence_registry, result_ref, backend,
    ...
    params = RenderVideo(result_ref=result_ref,backend=backend,t_start_s=t_start_s,t_end_s=t_end_s,fps=fps,
        azimuth_deg=azimuth_deg,elevation_deg=elevation_deg).model_dump()
    ...
    family=result.get('data',{}).get('contract')=='family.backend_data'
    adapter = ('family_mujoco_saved_v1' if family else 'mujoco_saved_qpos_v1') if backend=='mujoco' else MATLAB_ADAPTERS.get(model_id)
    if adapter is None:
        raise ValueError('UNSUPPORTED_RENDER_ADAPTER: '+str(model_id))
    if adapter=='tdcr_planar_saved_v1' and (azimuth_deg!=135. or elevation_deg!=-20.):
        raise ValueError('VIEWPOINT_UNSUPPORTED: legacy planar Figure has a fixed view')
    code = ['tools/simulation_video.py','tools/native_replay.py','tools/matlab_replay.py',
            'matlab/tdcr_replay_saved.m','matlab/encode_saved_frames.m','matlab/tf_view.m','extensions/tendon_family/saved.py']
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix='frames_',dir=output) as temporary:
            ...
            if backend=='mujoco':
                _mujoco_frames(folder,rows,indices,frames,azimuth_deg,elevation_deg)
            else:
                from tools.matlab_replay import capture_tdcr
                from extensions.tendon_family.saved import capture_matlab
                if adapter=='family_matlab_saved_v1':capture_matlab(engine,folder,frames,indices,fps,azimuth_deg,elevation_deg)
                else:capture_tdcr(engine,folder,frames,indices,fps)
    ...
    atomic_json(manifest,dict(**identity,receipt=receipt,source_indices=indices,
                ...
                render_settings=dict(format='mp4',width=800,height=600,azimuth_deg=azimuth_deg,elevation_deg=elevation_deg),
                state_reconstruction='mj_forward on saved named qpos/qvel or MATLAB tf_view on saved body poses; no integration',
