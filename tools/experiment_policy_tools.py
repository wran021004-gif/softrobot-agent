...
def validate_experiment_policy(path) -> ValidatedExperiment:
    ...
    if not path.is_relative_to(owner_root.resolve()):
    ...
    inputs = {p: file_hash(p) for p in (path, approval, baseline_path, resolved.manifest_path, *resolved.source_paths.values())}
    result = ValidatedExperiment(path, policy, file_hash(path), baseline, resolved, inputs)
