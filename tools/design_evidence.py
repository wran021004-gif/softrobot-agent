...
def refresh_index(book):
    ...
    for eid, ref in list(state['evidence'].items()):
        ...
        entry = dict(evidence_id=eid, candidate_id=ref.get('candidate_id'),
                     evaluation_id=ref.get('evaluation_id'), path=ref['path'],
                     readable=path.suffix == '.json', bytes=path.stat().st_size)
        if entry['readable']:
            value = read(path)
            entry['fields'] = list(value) if isinstance(value, dict) else ['array' if isinstance(value, list) else 'scalar']
...
def read_slice(root, state, arguments):
    ...
    start, cap = arguments.get('byte_offset', 0), arguments.get('max_bytes', 3000)
    ...
    return dict(evidence_id=eid, candidate_id=ref.get('candidate_id'),
                ...
                pointer=pointer, offset=offset, total_items=total,
